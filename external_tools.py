...
class MaskedAnalysisPipeline:
    ...
    def create_integrated_gene_table(self, dataset_names: List[str]) -> Dict:
        """
        ...
        # Format for output
        top_genes = []
        for masked, evidence in sorted_genes[:100]:
            gene_record = {
                'gene_id': masked,
    ...
    def format_for_llm_prompt(self, context: Dict, task_description: str = "") -> str:
        """
        ...
        if 'integrated_priority_table' in context:
            ...
            for g in context['integrated_priority_table']['top_priority_genes'][:30]:
                lines.append(f"| {g['gene_id']} | {g['priority_score']:.1f} | {g['n_evidence_types']} | "
                           f"{g['best_de_rank'] or '-'} | {g['mean_log2fc'] or '-'} | "
                           f"{g['n_pathways']} | {g['wgcna_best_role'] or '-'} | {g['n_regulating_tfs']} |")
            lines.append("")
    ...
    def save_all(self, output_dir: str, dataset_names: Optional[List[str]] = None):
        ...
        prompt = self.format_for_llm_prompt(context)
...
def run_full_pipeline(datasets: Dict[str, ad.AnnData],
    ...
    print("-" * 40)
    pipeline.create_integrated_gene_table(dataset_names)
