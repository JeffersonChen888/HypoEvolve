...
# Model configuration
MODEL_NAME = os.getenv("OPENAI_MODEL", "gpt-5")  # Default to GPT-5
REASONING_EFFORT = os.getenv("REASONING_EFFORT", "medium")  # Options: minimal, low, medium, high

def gpt4o_generate(prompt: str, max_tokens: int = 2000, temperature: float = 0.7, model: Optional[str] = None, reasoning_effort: Optional[str] = None) -> str:
    ...
    Args:
        ...
        model: Model name to use (default: None, uses MODEL_NAME).
               Options: 'o3-mini', 'o3', 'gpt-5', 'gpt-4o', 'gemini-2.5-pro', 'gemini-2.0-flash'
    ...
    try:
        ...
        # GPT-5 models use temperature=1 only, no token limit (let them think and respond fully)
            ...
            api_params["temperature"] = 1.0
            logging.info(f"Using GPT-5 model (temperature fixed at 1.0, no token limit)")
        else:
        ...
        # GPT-5: $1.25/1M input, $10.00/1M output (50% cheaper input than GPT-4o)
        # GPT-5-mini: $0.10/1M input, $0.50/1M output (estimated)
        ...
        if 'o3-mini' in selected_model or 'o1-mini' in selected_model:
            ...
            if 'mini' in selected_model or 'nano' in selected_model:
