...
3. **EvolutionAgent** (Crossover + Mutation)
...
4. **SupervisorAgent** (Workflow Orchestration)
...
5. **TournamentAgent** (lethal_genes_tournament mode only)
   - Swiss-style cross-pair ELO ranking
   - Used for Phase 2 of lethal genes tournament

### Removed Components

- **RankingAgent**: Replaced by fitness-based selection with local tournament
...
2. For each generation:
   ...
   │   ├── Preserve elites
   │   └── Local tournament selection (sample 2, pick fitter)
   │
...
Selection is implemented in `SupervisorAgent._tournament_select()`. Alternative methods can be added there:

```python
def _roulette_wheel_select(self, population):
    # Implement roulette wheel selection based on fitness_score
    pass
```
