#!/usr/bin/env python3
"""
Batch 1: Process prompts 1-5 (KLF5_ARID1A through WRN_microsatellite_instability_MSI)

Usage:
    python pipeline3/scripts/run_lethal_genes_2_batch1.py [--population-size N] [--generations N] [--model MODEL]

Example:
    python pipeline3/scripts/run_lethal_genes_2_batch1.py --population-size 6 --generations 3 --model gpt-4o
"""
...
# Batch configuration
BATCH_NUMBER = 1
START_INDEX = 0  # 0-indexed
END_INDEX = 5    # Exclusive (processes prompts 0-4, which are 1-5 in 1-indexed)
...
def main():
    ...
    if args.run_id:
        ...
        logging.info(f"Created new run_id: {run_id}")

    # Save run_id to file for other batches to use
    run_id_file = project_root / "pipeline3" / "output" / "lethal_genes_2" / ".last_run_id"
    try:
        with open(run_id_file, 'w') as f:
            f.write(run_id)
        logging.info(f"Saved run_id to: {run_id_file}")
    except Exception as e:
        logging.warning(f"Could not save run_id to file: {e}")
